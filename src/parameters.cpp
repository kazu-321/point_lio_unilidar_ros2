...
void readParameters(shared_ptr<rclcpp::Node> &nh)
{
  ...
  // nh->declare_param<bool>("prop_at_freq_of_imu", prop_at_freq_of_imu, 1);
  // nh->declare_param<bool>("use_imu_as_input", use_imu_as_input, 1);
  // nh->declare_param<bool>("check_satu", check_satu, 1);
  // nh->declare_param<int>("init_map_size", init_map_size, 100);
  // nh->declare_param<bool>("space_down_sample", space_down_sample, 1);
  // nh->declare_param<double>("mapping/satu_acc", satu_acc, 3.0);
  // nh->declare_param<double>("mapping/satu_gyro", satu_gyro, 35.0);
  // nh->declare_param<double>("mapping/acc_norm", acc_norm, 1.0);
  // nh->declare_param<float>("mapping/plane_thr", plane_thr, 0.05f);
  // nh->declare_param<int>("point_filter_num", p_pre->point_filter_num, 2);
  // nh->declare_param<std::string>("common/lid_topic", lid_topic, "/livox/lidar");
  // nh->declare_param<std::string>("common/imu_topic", imu_topic, "/livox/imu");
  // nh->declare_param<bool>("common/con_frame", con_frame, false);
  // nh->declare_param<int>("common/con_frame_num", con_frame_num, 1);
  // nh->declare_param<bool>("common/cut_frame", cut_frame, false);
  // nh->declare_param<double>("common/cut_frame_time_interval", cut_frame_time_interval, 0.1);
  // nh->declare_param<double>("common/time_lag_imu_to_lidar", time_lag_imu_to_lidar, 0.0);
  // nh->declare_param<double>("filter_size_surf", filter_size_surf_min, 0.5);
  // nh->declare_param<double>("filter_size_map", filter_size_map_min, 0.5);
  // nh->declare_param<double>("cube_side_length", cube_len, 200);
  // nh->declare_param<float>("mapping/det_range", DET_RANGE, 300.f);
  // nh->declare_param<double>("mapping/fov_degree", fov_deg, 180);
  // nh->declare_param<bool>("mapping/imu_en", imu_en, true);
  // nh->declare_param<bool>("mapping/start_in_aggressive_motion", non_station_start, false);
  // nh->declare_param<bool>("mapping/extrinsic_est_en", extrinsic_est_en, true);
  // nh->declare_param<double>("mapping/imu_time_inte", imu_time_inte, 0.005);
  // nh->declare_param<double>("mapping/lidar_meas_cov", laser_point_cov, 0.1);
  // nh->declare_param<double>("mapping/acc_cov_input", acc_cov_input, 0.1);
  // nh->declare_param<double>("mapping/vel_cov", vel_cov, 20);
  // nh->declare_param<double>("mapping/gyr_cov_input", gyr_cov_input, 0.1);
  // nh->declare_param<double>("mapping/gyr_cov_output", gyr_cov_output, 0.1);
  // nh->declare_param<double>("mapping/acc_cov_output", acc_cov_output, 0.1);
  // nh->declare_param<double>("mapping/b_gyr_cov", b_gyr_cov, 0.0001);
  // nh->declare_param<double>("mapping/b_acc_cov", b_acc_cov, 0.0001);
  // nh->declare_param<double>("mapping/imu_meas_acc_cov", imu_meas_acc_cov, 0.1);
  // nh->declare_param<double>("mapping/imu_meas_omg_cov", imu_meas_omg_cov, 0.1);
  // nh->declare_param<double>("preprocess/blind", p_pre->blind, 1.0);
  // nh->declare_param<int>("preprocess/lidar_type", lidar_type, 1);
  // nh->declare_param<int>("preprocess/scan_line", p_pre->N_SCANS, 16);
  // nh->declare_param<int>("preprocess/scan_rate", p_pre->SCAN_RATE, 10);
  // nh->declare_param<int>("preprocess/timestamp_unit", p_pre->time_unit, 1);
  // nh->declare_param<double>("mapping/match_s", match_s, 81);
  // nh->declare_param<bool>("mapping/gravity_align", gravity_align, true);
  // nh->declare_param<std::vector<double>>("mapping/gravity", gravity, std::vector<double>());
  // nh->declare_param<std::vector<double>>("mapping/gravity_init", gravity_init, std::vector<double>());
  // nh->declare_param<std::vector<double>>("mapping/extrinsic_T", extrinT, std::vector<double>());
  // nh->declare_param<std::vector<double>>("mapping/extrinsic_R", extrinR, std::vector<double>());
  // nh->declare_param<bool>("odometry/publish_odometry_without_downsample", publish_odometry_without_downsample, false);
  // nh->declare_param<bool>("publish/path_en", path_en, true);
  // nh->declare_param<bool>("publish/scan_publish_en", scan_pub_en, 1);
  // nh->declare_param<bool>("publish/scan_bodyframe_pub_en", scan_body_pub_en, 1);
  // nh->declare_param<bool>("runtime_pos_log_enable", runtime_pos_log, 0);
  // nh->declare_param<bool>("pcd_save/pcd_save_en", pcd_save_en, false);
  // nh->declare_param<int>("pcd_save/interval", pcd_save_interval, -1);
}
